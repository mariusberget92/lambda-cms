# SelectBox Component Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build a reusable `SelectBox.vue` dropdown component supporting single/multi-select, optional search, and a clear button.

**Architecture:** Single self-contained SFC at `resources/js/Components/SelectBox.vue`. Pure Vue 3 with `onClickOutside` from `@vueuse/core` (already installed). No external UI primitives. The dropdown uses `position: absolute` inside a `position: relative` root — no Teleport needed.

**Tech Stack:** Vue 3 Composition API, `@vueuse/core`, Tailwind CSS 4 (Nord theme CSS variables)

---

### Task 1: Create `SelectBox.vue`

**Files:**
- Create: `resources/js/Components/SelectBox.vue`

No frontend test runner is configured in this project, so verification is done via build success + visual inspection in the browser.

---

**Step 1: Create the file with the full implementation**

Create `resources/js/Components/SelectBox.vue` with the following content:
...
Expected: `✓ built in X.XXs` with no errors. The only expected warning is the pre-existing chunk size notice.
...
```bash
git add resources/js/Components/SelectBox.vue
git commit -m "feat: add SelectBox component (single/multi, searchable, clearable)"
```
