<template>
  <header class="flex items-center gap-3 px-3 h-11 shrink-0 border-b border-border bg-card z-10">
    ...
    <!-- Type badge -->
    <span class="shrink-0 px-2 py-0.5 rounded text-[11px] font-semibold uppercase tracking-wider bg-muted text-muted-foreground">
      {{ typeLabel }}
    </span>
    ...
    <!-- Loop source (only for non-single-post template types) -->
    <template v-if="showLoopSource">
      <div class="w-px h-5 bg-border shrink-0" />
      <div class="flex items-center gap-1.5 shrink-0">
        <span class="text-xs text-muted-foreground">Source</span>
        <select
          :value="loopSource"
          class="bg-muted/50 border border-border rounded px-2 py-1 text-xs text-foreground/80 focus:outline-none focus:border-muted-foreground transition-colors [color-scheme:dark]"
          @change="$emit('update:loop-source', $event.target.value)"
        >
...
defineProps({
  ...
  showLoopSource:   { type: Boolean, default: false },
  status:           { type: String,  default: 'draft' },
  metaTitle:        { type: String,  default: '' },
  metaDescription:  { type: String,  default: '' },
  metaKeywords:     { type: String,  default: '' },
  processing:       { type: Boolean, default: false },
  saveLabel:        { type: String,  default: 'Save template' },
...
const emit = defineEmits([
  'update:title', 'update:loop-source', 'update:status',
